...
//            Log.d("test", characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT16, 0).toString())
            var count: Int = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT16, 0)
            with(characteristic) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        Timber.i("Read characteristic $uuid | value: ${count}")
